 ...
 * can be specified via the #GstProgressReport2:update-freq and the
 * #GstProgressReport2:format property.
 *
 ...
 * terminal. This can be prevented by setting the #GstProgressReport2:silent
 * property to %TRUE.
 *
...
GstStaticPadTemplate progress_report_2_src_template =
GST_STATIC_PAD_TEMPLATE ("src",
    GST_PAD_SRC,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS_ANY);

GstStaticPadTemplate progress_report_2_sink_template =
...
#define DEFAULT_HIGH_PRECISION FALSE
#define DEFAULT_ABSOLUTE_TIME FALSE
#define DEFAULT_FORMAT       "auto"

static void gst_progress_report_2_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_progress_report_2_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

static gboolean gst_progress_report_2_event (GstBaseTransform * trans,
    GstEvent * event);
static GstFlowReturn gst_progress_report_2_transform_ip (GstBaseTransform * trans,
    GstBuffer * buf);

static gboolean gst_progress_report_2_start (GstBaseTransform * trans);
static gboolean gst_progress_report_2_stop (GstBaseTransform * trans);

GST_BOILERPLATE (GstProgressReport2, gst_progress_report_2, GstBaseTransform,
    GST_TYPE_BASE_TRANSFORM);

static void
gst_progress_report_2_base_init (gpointer g_class)
{
  GstElementClass *element_class = GST_ELEMENT_CLASS (g_class);

  gst_element_class_add_pad_template (element_class,
      gst_static_pad_template_get (&progress_report_2_sink_template));
  gst_element_class_add_pad_template (element_class,
      gst_static_pad_template_get (&progress_report_2_src_template));
...
static void
gst_progress_report_2_finalize (GObject * obj)
{
  GstProgressReport2 *filter = GST_PROGRESS_REPORT_2 (obj);
...
static void
gst_progress_report_2_class_init (GstProgressReport2Class * g_class)
{
  ...
  gobject_class->finalize = gst_progress_report_2_finalize;
  gobject_class->set_property = gst_progress_report_2_set_property;
  gobject_class->get_property = gst_progress_report_2_get_property;
  ...
  g_object_class_install_property (gobject_class,
      ARG_HIGH_PRECISION, g_param_spec_boolean ("high-precision",
          "Print time with as much precision as possible",
          "Print time with as much precision as possible",
          DEFAULT_HIGH_PRECISION, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class,
      ARG_ABSOLUTE_TIME, g_param_spec_boolean ("absolute-time",
          "Print time since UNIX epoch instead of time since stream start",
          "Print time since UNIX epoch instead of time since stream start",
          DEFAULT_ABSOLUTE_TIME, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  ...
  gstbasetrans_class->event = GST_DEBUG_FUNCPTR (gst_progress_report_2_event);
  gstbasetrans_class->transform_ip =
      GST_DEBUG_FUNCPTR (gst_progress_report_2_transform_ip);
  gstbasetrans_class->start = GST_DEBUG_FUNCPTR (gst_progress_report_2_start);
  gstbasetrans_class->stop = GST_DEBUG_FUNCPTR (gst_progress_report_2_stop);
}

static void
gst_progress_report_2_init (GstProgressReport2 * report,
    GstProgressReport2Class * g_class)
{
  ...
  report->high_precision = DEFAULT_HIGH_PRECISION;
  report->format = g_strdup (DEFAULT_FORMAT);
}

static void
gst_progress_report_2_post_progress (GstProgressReport2 * filter,
...
static gboolean
gst_progress_report_2_do_query (GstProgressReport2 * filter, GstFormat format,
    gint hh, gint mm, gint ss, gint us, GstBuffer * buf)
{
  ...
  switch (format) {
    ...
    case GST_FORMAT_TIME:
      format_name = "seconds";
      if (filter->high_precision) {
        format_name = "nanoseconds";
      } else {
        format_name = "seconds";
        cur /= GST_SECOND;
        total /= GST_SECOND;
  ...
  if (!filter->silent) {
    if (total > 0) {
      if (filter->high_precision)
        g_print ("%s (%02d:%02d:%02d.%06d): %" G_GINT64_FORMAT " / %"
            G_GINT64_FORMAT " %s (%4.1f %%)\n", GST_OBJECT_NAME (filter), hh,
            mm, ss, us, cur, total, format_name, (gdouble) cur / total * 100.0);
      else
      ...
      if (filter->high_precision)
        g_print ("%s (%02d:%02d:%02d.%06d): %" G_GINT64_FORMAT " %s\n",
            GST_OBJECT_NAME (filter), hh, mm, ss, us, cur, format_name);
      else
  ...
  gst_progress_report_2_post_progress (filter, format, cur, total);
  return TRUE;
}

static void
gst_progress_report_2_report (GstProgressReport2 * filter, GTimeVal cur_time,
    GstBuffer * buf)
{
  ...
  run_time = (GstClockTime) cur_time.tv_sec * 1000000 + cur_time.tv_usec;
  if (!(filter->absolute_time))
    run_time -= (GstClockTime) filter->start_time.tv_sec * 1000000 +
        filter->start_time.tv_usec;

  hh = (run_time / 3600000000);
  if (!(filter->absolute_time))
    hh %= 100;
  mm = (run_time / 60000000) % 60;
  ss = (run_time / 1000000) % 60;
  us = (run_time % 1000000);
  ...
  if (format != GST_FORMAT_UNDEFINED) {
    done = gst_progress_report_2_do_query (filter, format, hh, mm, ss, us, buf);
  } else {
    gint i;

    for (i = 0; i < G_N_ELEMENTS (try_formats); ++i) {
      done = gst_progress_report_2_do_query (filter, try_formats[i], hh, mm, ss,
          us, buf);
      if (done)
        break;
    }
  ...
  if (!done && !filter->silent) {
    if (filter->high_precision)
      g_print ("%s (%2d:%2d:%2d.%06d): Could not query position and/or duration\n",
          GST_OBJECT_NAME (filter), hh, mm, ss, us);
    else
...
static gboolean
gst_progress_report_2_event (GstBaseTransform * trans, GstEvent * event)
{
  GstProgressReport2 *filter;

  filter = GST_PROGRESS_REPORT_2 (trans);

  if (GST_EVENT_TYPE (event) == GST_EVENT_EOS) {
    GTimeVal cur_time;

    g_get_current_time (&cur_time);
    gst_progress_report_2_report (filter, cur_time, NULL);
  }
  return GST_BASE_TRANSFORM_CLASS (parent_class)->event (trans, event);
}

static GstFlowReturn
gst_progress_report_2_transform_ip (GstBaseTransform * trans, GstBuffer * buf)
{
  GstProgressReport2 *filter;
  ...
  if (need_update) {
    gst_progress_report_2_report (filter, cur_time, buf);
    GST_OBJECT_LOCK (filter);
...
static gboolean
gst_progress_report_2_start (GstBaseTransform * trans)
{
  GstProgressReport2 *filter;
...
static gboolean
gst_progress_report_2_stop (GstBaseTransform * trans)
{
...
static void
gst_progress_report_2_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstProgressReport2 *filter;

  filter = GST_PROGRESS_REPORT_2 (object);
  ...
  switch (prop_id) {
    ...
    case ARG_DO_QUERY:
      ...
      GST_OBJECT_UNLOCK (filter);
      break;
    case ARG_HIGH_PRECISION:
      GST_OBJECT_LOCK (filter);
      filter->high_precision = g_value_get_boolean (value);
      GST_OBJECT_UNLOCK (filter);
      break;
    case ARG_ABSOLUTE_TIME:
      GST_OBJECT_LOCK (filter);
      filter->absolute_time = g_value_get_boolean (value);
      GST_OBJECT_UNLOCK (filter);
      break;
    case ARG_FORMAT:
      GST_OBJECT_LOCK (filter);
...
static void
gst_progress_report_2_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstProgressReport2 *filter;
  ...
  switch (prop_id) {
    ...
    case ARG_HIGH_PRECISION:
      GST_OBJECT_LOCK (filter);
      g_value_set_boolean (value, filter->high_precision);
      GST_OBJECT_UNLOCK (filter);
      break;
    case ARG_ABSOLUTE_TIME:
      GST_OBJECT_LOCK (filter);
      g_value_set_boolean (value, filter->absolute_time);
      GST_OBJECT_UNLOCK (filter);
      break;
    case ARG_FORMAT:
      GST_OBJECT_LOCK (filter);
...
{
  return gst_element_register (plugin, "progressreport2", GST_RANK_NONE,
      GST_TYPE_PROGRESS_REPORT_2);
}
