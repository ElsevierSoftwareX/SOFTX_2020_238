...
static void increment_bin(CairoVisHistogram *element, double x, gchar value)
{
	switch (element->bins)
	{
		...
		case CAIROVIS_BINS_LOG:
			x = element->nbins * (log2(x) - log2(element->min)) / (log2(element->max) - log2(element->min));
			break;
	}
...
static void increment_bin_from_ptr(CairoVisHistogram *element, const double *ptr, const double *end, gchar value)
{
	for(; ptr < end; ptr ++)
...
static void increment_bin_from_buf(CairoVisHistogram *element, GstBuffer *buf, gchar value)
{
	const double *ptr = (const double *) GST_BUFFER_DATA(buf);
	const double *end = (const double *) (GST_BUFFER_DATA(buf) + GST_BUFFER_SIZE(buf));
...
static GstFlowReturn chain(GstPad *pad, GstBuffer *inbuf)
{
	...
	{
		GstBuffer *buf = gst_adapter_take_buffer(element->adapter, available_bytes - history_bytes);
...
static void set_property(GObject * object, enum property id, const GValue * value, GParamSpec * pspec)
{
	...
	if (id == PROP_BINS || id == PROP_BIN_MIN || id == PROP_BIN_MAX || id == PROP_NUM_BINS)
	{
		g_free(element->bin_counts);
		element->bin_counts = NULL;
...
GType cairovis_histogram_get_type(void)
{
	...
	if(!type) {
		...
		type = g_type_register_static(CAIROVIS_BASE_TYPE, "cairovis_histogram", &info, 0);
	}
