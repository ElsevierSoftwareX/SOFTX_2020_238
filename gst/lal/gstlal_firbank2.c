 ...
 *
 * Modified by Leo Singer <leo.singer@ligo.org> in 2011 from gstlal_firbank.h
 * to emit the start-up transient (so that input segment and output segment
 * are the same), and to not have the 'latency' property which has a different
 * purpose than the 'latency' property of audiofirfilter.
 *
...
static unsigned fir_channels(const GSTLALFIRBank2 *element)
{
...
static unsigned fir_length(const GSTLALFIRBank2 *element)
{
...
static unsigned fft_block_length(const GSTLALFIRBank2 *element)
{
...
static unsigned fft_block_stride(const GSTLALFIRBank2 *element)
{
...
static guint64 minimum_input_length(const GSTLALFIRBank2 *element, guint64 n)
{
...
static int push_zeros(GSTLALFIRBank2 *element, unsigned samples)
{
...
static void set_metadata(GSTLALFIRBank2 *element, GstBuffer *buf, guint64 outsamples, gboolean gap)
{
...
static guint64 get_available_samples(GSTLALFIRBank2 *element)
{
	return gst_adapter_available(element->adapter) / sizeof(double);
}


static guint64 get_available_nonzero_samples(GSTLALFIRBank2 *element)
{
...
static int create_fft_workspace(GSTLALFIRBank2 *element)
{
	unsigned i;
...
static void free_fft_workspace(GSTLALFIRBank2 *element)
{
	g_mutex_lock(gstlal_fftw_lock);
...
static unsigned tdfilter(GSTLALFIRBank2 *element, GstBuffer *outbuf, unsigned available_length)
{
	unsigned i;
...
static unsigned fdfilter(GSTLALFIRBank2 *element, GstBuffer *outbuf, unsigned available_length)
{
	unsigned stride;
	unsigned blocks;
...
static GstFlowReturn filter(GSTLALFIRBank2 *element, GstBuffer *outbuf)
{
...
static GstFlowReturn filter_and_push(GSTLALFIRBank2 *element, guint64 length)
{
...
{
	GSTLALFIRBank2 *element = GSTLAL_FIRBANK2(trans);
	guint n;
...
{
	GSTLALFIRBank2 *element = GSTLAL_FIRBANK2(object);
...
static void gstlal_firbank2_base_init(gpointer gclass)
{
	GstElementClass *element_class = GST_ELEMENT_CLASS(gclass);
	GstBaseTransformClass *transform_class = GST_BASE_TRANSFORM_CLASS(gclass);

	gst_element_class_set_details_simple(element_class, "FIR Filter Bank", "Filter/Audio", "Projects a single audio channel onto a bank of FIR filters to produce a multi-channel output.\nUnlike lal_firbank, emits buffers for the initial filter transient so that the input and output segments match.", "Kipp Cannon <kipp.cannon@ligo.org>, Leo Singer <leo.singer@ligo.org>");
...
static void gstlal_firbank2_class_init(GSTLALFIRBank2Class *klass)
{
...
static void gstlal_firbank2_init(GSTLALFIRBank2 *filter, GSTLALFIRBank2Class *kclass)
{
