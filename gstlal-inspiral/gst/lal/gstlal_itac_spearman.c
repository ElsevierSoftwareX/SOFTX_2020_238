/*
 * An inspiral trigger and autocorrelation chisq plus spearman ranking element
 *
 * Copyright (C) 2015 Linqing Wen
 *
 *  THis program is modified from the original gstlal_itac.c by 
...
/*
 * our own stuff
 * DEBUG WEN: xx_spearman replace the original file by adding spearman component
 */
...
/* DEBUG WEN */
#include <gstlal/gstlal_spearman_pval.h>
#include <gstlal_snglinspiral.h>
#include <gstlal_snglinspiral_spearman.h>
...
static unsigned autocorrelation_channels(const GSTLALItacSpearman *element)
{
	return gstlal_autocorrelation_chi2_autocorrelation_channels(element->autocorrelation_matrix);
}


static unsigned autocorrelation_length(const GSTLALItacSpearman *element)
{
	return gstlal_autocorrelation_chi2_autocorrelation_length(element->autocorrelation_matrix);
}


static guint64 output_num_samps(GSTLALItacSpearman *element)
{
	return (guint64) element->n;
}


static guint64 output_num_bytes(GSTLALItacSpearman *element)
{
	return (guint64) output_num_samps(element) * element->adapter->unit_size;
}


static int reset_time_and_offset(GSTLALItacSpearman *element)
{
...
static void free_bank(GSTLALItacSpearman *element)
{
...
static void update_peak_info_from_autocorrelation_properties(GSTLALItacSpearman *element)
{
...
static GstFlowReturn process(GSTLALItacSpearman *element);

static gboolean sink_event(GstPad *pad, GstEvent *event)
{
	GSTLALItacSpearman *element = GSTLAL_ITAC_SPEARMAN(GST_PAD_PARENT(pad));
...
{
	GSTLALItacSpearman *element = GSTLAL_ITAC_SPEARMAN(object);
...
{
	GSTLALItacSpearman *element = GSTLAL_ITAC_SPEARMAN(gst_pad_get_parent(pad));
	...
	if(success) {
		...
		if (width == 128) {
			...
			element->pval = calloc(element->channels, sizeof(double));
			}
		if (width == 64) {
			element->peak_type = GSTLAL_PEAK_COMPLEX;
			element->chi2 = calloc(element->channels, sizeof(float));
			element->pval = calloc(element->channels, sizeof(float));
			}
		if (element->maxdata)
...
static void update_state(GSTLALItacSpearman *element, GstBuffer *srcbuf)
{
...
static GstFlowReturn push_buffer(GSTLALItacSpearman *element, GstBuffer *srcbuf)
{
...
static GstFlowReturn push_gap(GSTLALItacSpearman *element, guint samps)
{
	...
	srcbuf = gstlal_snglinspiral_new_buffer_from_peak_spearman(element->maxdata, element->bankarray, element->srcpad, element->next_output_offset, samps, element->next_output_timestamp, element->rate, NULL, NULL);
...
static GstFlowReturn push_nongap(GSTLALItacSpearman *element, guint copysamps, guint outsamps)
{
	GstBuffer *srcbuf = NULL;
	GstFlowReturn result = GST_FLOW_OK;
	union {
	...
	if (element->autocorrelation_matrix) {
		...
		if (element->peak_type == GSTLAL_PEAK_DOUBLE_COMPLEX) {
			...
			/* FIXME: not adding double spearman ranking yet - should be trivial */
			}
		if (element->peak_type == GSTLAL_PEAK_COMPLEX) {
			/* extract data around peak for chisq calculation */
			...
			/* DEBUG WEN */
			gstlal_spearman_float((float *) element->pval, (float complex *) element->snr_mat, autocorrelation_length(element), -((int) autocorrelation_length(element)) / 2, 0.0, element->autocorrelation_matrix, element->autocorrelation_mask, element->autocorrelation_norm);
			}
		/* create the output buffer */
		srcbuf = gstlal_snglinspiral_new_buffer_from_peak_spearman(element->maxdata, element->bankarray, element->srcpad, element->next_output_offset, outsamps, element->next_output_timestamp, element->rate, element->chi2, element->pval);
		}
	else
	  srcbuf = gstlal_snglinspiral_new_buffer_from_peak_spearman(element->maxdata, element->bankarray, element->srcpad, element->next_output_offset, outsamps, element->next_output_timestamp, element->rate, NULL, NULL);
...
static void base_init(gpointer class)
{
	...
	gst_element_class_set_details_simple(
		...
		"Linqing Wen  <linqing.wen@ligo.org>"
	);
...
static void class_init(gpointer class, gpointer class_data)
{
	...
	g_object_class_install_property(
		...
		g_param_spec_uint(
			"n",
			"n",
			"number of samples over which to identify itacs_spearman",
			0, G_MAXUINT, 0,
...
/*
 * gstlal_itac_spearman_get_type().
 */


GType gstlal_itac_spearman_get_type(void)
{
	static GType type = 0;

	if(!type) {
		static const GTypeInfo info = {
			.class_size = sizeof(GSTLALItacSpearmanClass),
			.class_init = class_init,
			.base_init = base_init,
			.instance_size = sizeof(GSTLALItacSpearman),
			.instance_init = instance_init,
		};
		type = g_type_register_static(GST_TYPE_ELEMENT, "GSTLALItacSpearman", &info, 0);
	}
