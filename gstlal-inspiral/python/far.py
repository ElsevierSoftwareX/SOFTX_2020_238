...
class ThincaCoincParamsDistributions():
	...
	# this class
	SNRPDF = 0#inspiral_extrinsics.SNRPDF.load()
...
#		self.injection_rates = dict((param, rate.BinnedArray(binning)) for param, binning in self.binnings.items())
	...
	def denominator(self, params):
		...
		lnP_dt_dphi_noise = 0#inspiral_extrinsics.lnP_dt_dphi(params, self.delta_t, model = "noise")
	...
	def from_xml(cls, xml, name):
		...
		def reconstruct(xml, prefix, target_dict):
			for name in [elem.Name.split(u":")[1] for elem in xml.childNodes if elem.Name.startswith(u"%s:" % prefix)]:
				target_dict[str(name)] = None #rate.BinnedArray.from_xml(xml, u"%s:%s" % (prefix, name))
...
class RankingData(object):
	...
	binnings = {
		"ln_likelihood_ratio": None #rate.NDBins((rate.ATanBins(0., 110., 3000),))
	}

	filters = {
		"ln_likelihood_ratio": None #rate.gaussian_window(4.)
	}
	...
	def from_xml(cls, xml, name):
		...
		def reconstruct(xml, prefix, target_dict):
			for ba_elem in [elem for elem in xml.getElementsByTagName(ligolw.LIGO_LW.tagName) if elem.hasAttribute(u"Name") and ("_%s" % prefix) in elem.Name]:
				...
				target_dict[ifo_set] = 0#rate.BinnedArray.from_xml(ba_elem, ba_elem.Name.split(":")[0])
