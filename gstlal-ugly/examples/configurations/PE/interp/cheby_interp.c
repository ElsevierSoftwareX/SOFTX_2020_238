...
double onedCheby(double x, int J, int J_max) {
	double w;
...
double twodCheby(double x, int K, int K_max, double y, int L, int L_max) {
...
gsl_matrix_complex * compute_C_KL(gsl_vector *x_k, gsl_vector *y_l, gsl_matrix_complex *M) {
	...
	double	k_max = x_k->size;
	double  l_max = y_l->size;
...
gsl_vector *interpolate_waveform_from_mchirp_and_eta(struct twod_waveform_interpolant_array *interps, double mchirp, double eta) { 
	int i;
	gsl_complex M;
	double deltaF, x, y;
	gsl_vector_complex *h_f = gsl_vector_complex_calloc(interps[0]->svd_basis->size);
