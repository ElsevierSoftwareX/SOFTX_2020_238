...
def build_and_run(pipelinefunc, name, segment = None, **pipelinefunc_kwargs):
	...
	#handler = MultiChannelHandler(mainloop, pipeline, output = "output.txt", instrument = instrument)
	handler = MultiChannelHandler(mainloop, pipeline, output = "")
...
def peak_test_01a(pipeline,name):
	#
	# try changing these.  test should still work!
	#

	print >>sys.stderr, "=== Checkpoint #1a ==="
	initial_channels = 2
	rate = 2048	#Hz
	width = 32
	sine_frequency = 1
	...
	print >>sys.stderr, "=== Checkpoint #1b ==="
	...
	print >>sys.stderr, "=== Checkpoint #1f ==="
	...
	print >>sys.stderr, "=== Checkpoint #1g ==="
	#sink.connect("new-sample",  handler.bufhandler)
	print >>sys.stderr, "=== Checkpoint #1h ==="
	#sink.connect("new-preroll", handler.prehandler)
	print >>sys.stderr, "=== Checkpoint #1i ==="
