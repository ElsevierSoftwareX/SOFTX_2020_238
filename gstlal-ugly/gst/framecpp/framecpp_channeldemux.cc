...
static GstFlowReturn chain(GstPad *pad, GstBuffer *inbuf)
{
	...
	try {
		...
		snprintf(container_format, sizeof(container_format), container_format, ifs.Version());
		gst_tag_list_add(tag_list, GST_TAG_MERGE_APPEND, GST_TAG_CONTAINER_FORMAT, container_format, GST_TAG_ENCODER, get_frame_library_name(&ifs), GST_TAG_ENCODER_VERSION, ifs.LibraryRevision(), NULL);
		}
