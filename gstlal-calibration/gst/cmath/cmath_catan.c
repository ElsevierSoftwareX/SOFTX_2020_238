...
#define TYPE_CMATH_CATAN \
	(cmath_catan_get_type())
#define CMATH_CATAN(obj) \
	(G_TYPE_CHECK_INSTANCE_CAST((obj),TYPE_CMATH_CATAN,CMathCATan))
#define CMATH_CATAN_CLASS(klass) \
	(G_TYPE_CHECK_CLASS_CAST((klass),TYPE_CMATH_CATAN,CMathCATanClass))
#define IS_PLUGIN_TEMPLATE(obj) \
	(G_TYPE_CHECK_INSTANCE_TYPE((obj),TYPE_CMATH_CATAN))
#define IS_PLUGIN_TEMPLATE_CLASS(klass) \
	(G_TYPE_CHECK_CLASS_TYPE((klass),TYPE_CMATH_CATAN))

typedef struct _CMathCATan CMathCATan;
typedef struct _CMathCATanClass CMathCATanClass;

GType
cmath_catan_get_type(void);
...
{
	CMathCATan *element = CMATH_CATAN(trans);
...
static void
cmath_catan_class_init(gpointer klass, gpointer klass_data)
{
	...
	gst_element_class_set_details_simple(GST_ELEMENT_CLASS(klass),
		"Arctangent",
		"Filter/Audio",
		"Calculate the arctangent, y = arctan(x)",
...
cmath_catan_get_type(void)
{
	...
	if(!type) {
		static const GTypeInfo info = {
			...
			.class_init = cmath_catan_class_init,
			.instance_size = sizeof(CMathBase),
		};
		type = g_type_register_static(CMATH_BASE_TYPE, "CMathCATan", &info, 0);
	}
