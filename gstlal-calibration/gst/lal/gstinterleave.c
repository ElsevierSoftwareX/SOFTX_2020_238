...
#define GST_INTERLEAVE_PAD(pad) (G_TYPE_CHECK_INSTANCE_CAST((pad),GST_TYPE_INTERLEAVE_PAD,GstLALInterleavePad))
#define GST_INTERLEAVE_PAD_CAST(pad) ((GstLALInterleavePad *) pad)
...
gst_interleave_pad_get_type (void)
{
  ...
  if (G_UNLIKELY (type == 0)) {
    type = g_type_register_static_simple (GST_TYPE_PAD,
        g_intern_static_string ("GstLALInterleavePad"), sizeof (GstPadClass),
        (GClassInitFunc) gst_interleave_pad_class_init,
        sizeof (GstLALInterleavePad), NULL, 0);
  }
  return type;
}
...
{
  GstLALInterleavePad *self = GST_INTERLEAVE_PAD (object);
...
GST_BOILERPLATE (GstLALInterleave, gst_interleave, GstElement, GST_TYPE_ELEMENT);
...
{
  GstLALInterleave *self = GST_INTERLEAVE (object);
...
static void
gst_interleave_set_channel_positions (GstLALInterleave * self, GstStructure * s)
{
  GValue pos_array = { 0, };
  gint i;
...
static void
gst_interleave_class_init (GstLALInterleaveClass * klass)
{
  ...
  /* Reference GstLALInterleavePad class to have the type registered from
  ...
  /**
   * GstLALInterleave:channel-positions
   * 
  ...
  /**
   * GstLALInterleave:channel-positions-from-input
   * 
...
static void
gst_interleave_init (GstLALInterleave * self, GstLALInterleaveClass * klass)
{
...
{
  GstLALInterleave *self = GST_INTERLEAVE (element);
  GstPad *new_pad;
  gchar *pad_name;
...
gst_interleave_release_pad (GstElement * element, GstPad * pad)
{
  ...
  for (l = GST_ELEMENT_CAST (self)->sinkpads; l != NULL; l = l->next) {
    GstLALInterleavePad *ipad = GST_INTERLEAVE_PAD (l->data);
...
static void
gst_interleave_set_process_function (GstLALInterleave * self)
{
  switch (self->width) {
    case 8:
      self->func = (GstLALInterleaveFunc) interleave_8;
      break;
    case 16:
      self->func = (GstLALInterleaveFunc) interleave_16;
      break;
    case 24:
      self->func = (GstLALInterleaveFunc) interleave_24;
      break;
    case 32:
      self->func = (GstLALInterleaveFunc) interleave_32;
      break;
    case 64:
      self->func = (GstLALInterleaveFunc) interleave_64;
      break;
    default:
...
static gboolean
gst_interleave_src_query_duration (GstLALInterleave * self, GstQuery * query)
{
  gint64 max;
  gboolean res;
  GstFormat format;
...
static gboolean
gst_interleave_src_query_latency (GstLALInterleave * self, GstQuery * query)
{
...
static gboolean
forward_event (GstLALInterleave * self, GstEvent * event)
{
  GstIterator *it;
  GValue vret = { 0 };
...
static GstClockTime
output_timestamp_from_offset (const GstLALInterleave *self, guint64 offset)
{
  return self->segment.start + gst_util_uint64_scale_int_round (offset, GST_SECOND, self->rate);
}
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR, GST_VERSION_MINOR, "lal_interleave", "Audio interleaver", plugin_init, VERSION, "LGPL", GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN);
