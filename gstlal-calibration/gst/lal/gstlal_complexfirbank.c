...
#define GST_CAT_DEFAULT gstlal_complexfirbank_debug
GST_DEBUG_CATEGORY_STATIC(GST_CAT_DEFAULT);


G_DEFINE_TYPE_WITH_CODE(
	GSTLALComplexFIRBank,
	gstlal_complexfirbank,
	GST_TYPE_BASE_TRANSFORM,
	GST_DEBUG_CATEGORY_INIT(GST_CAT_DEFAULT, "lal_complexfirbank", 0, "lal_complexfirbank element")
);
...
static unsigned fir_channels(const GSTLALComplexFIRBank *element)
{
...
static unsigned fir_length(const GSTLALComplexFIRBank *element)
{
...
static unsigned fft_block_length(const GSTLALComplexFIRBank *element)
{
...
static unsigned fft_block_stride(const GSTLALComplexFIRBank *element)
{
...
static unsigned td_output_length(const GSTLALComplexFIRBank *element, unsigned input_length)
{
	/*
...
static unsigned get_output_length(const GSTLALComplexFIRBank *element, unsigned input_length)
{
...
static guint64 minimum_input_length(const GSTLALComplexFIRBank *element, guint64 n)
{
	if(element->time_domain)
...
static int push_gap(GSTLALComplexFIRBank *element, unsigned samples)
{
	GstBuffer *zerobuf;
...
static void set_metadata(GSTLALComplexFIRBank *element, GstBuffer *buf, guint64 outsamples, gboolean gap)
{
...
static guint64 get_available_samples(GSTLALComplexFIRBank *element)
{
	guint size;
...
static int create_tds_workspace(GSTLALComplexFIRBank *element)
{
	unsigned i, j;
...
static void free_tds_workspace(GSTLALComplexFIRBank *element)
{
...
static int create_fdd_workspace(GSTLALComplexFIRBank *element)
{
	unsigned i;
...
static void free_fdd_workspace(GSTLALComplexFIRBank *element)
{
	gstlal_fftw_lock();
...
static int create_fds_workspace(GSTLALComplexFIRBank *element)
{
	unsigned i;
	unsigned length_fd = fft_block_length(element) / 2 + 1;
...
static void free_fds_workspace(GSTLALComplexFIRBank *element)
{
	gstlal_fftw_lock();
...
static void free_workspace(GSTLALComplexFIRBank *element)
{
	if(!GST_AUDIO_INFO_IS_VALID(&element->audio_info))
		return;	/* assume workspace in not initialized */
	if(element->time_domain) {
		if(element->data_type == GSTLAL_COMPLEXFIRBANK_F64 || element->data_type == GSTLAL_COMPLEXFIRBANK_Z128)
			return;	/* no-op */
		else if(element->data_type == GSTLAL_COMPLEXFIRBANK_F32 || element->data_type == GSTLAL_COMPLEXFIRBANK_Z64)
...
static unsigned tddfilter(GSTLALComplexFIRBank *element, GstMapInfo *mapinfo, unsigned output_length)
{
	unsigned i;
...
static unsigned tddcfilter(GSTLALComplexFIRBank *element, GstMapInfo *mapinfo, unsigned output_length)
{
	unsigned i, j;
	unsigned input_length;
	double complex *input, *output;
	double *input_real, *input_imag, *output_real, *output_imag;
	gsl_vector_view input_view_real, input_view_imag;
	...
	input = g_malloc(input_length * sizeof(*input));
	output = (double complex *) mapinfo->data;
	input_real = g_malloc(input_length * sizeof(*input_real));
	input_imag = g_malloc(input_length * sizeof(*input_imag));
	output_real = g_malloc(output_length * sizeof(*output_real));
	output_imag = g_malloc(output_length * sizeof(*output_imag));
	gst_audioadapter_copy_samples(element->adapter, input, input_length, NULL, NULL);
	for(j = 0; j < input_length; j++) {
		input_real[j] = creal(input[j]);
		input_imag[j] = cimag(input[j]);
	}
	input_view_real = gsl_vector_view_array(input_real, fir_length(element));
	input_view_imag = gsl_vector_view_array(input_imag, fir_length(element));

	/*
	 * wrap real and imaginary parts of the output in a GSL matrix view.
	 */

	output_view_real = gsl_matrix_view_array(output_real, output_length, fir_channels(element));
	output_view_imag = gsl_matrix_view_array(output_imag, output_length, fir_channels(element));
	...
	for(i = 0; i < output_length; i++) {
		...
		gsl_vector_view output_sample_real = gsl_matrix_row(&output_view_real.matrix, i);
		gsl_vector_view output_sample_imag = gsl_matrix_row(&output_view_imag.matrix, i);
		...
		gsl_blas_dgemv(CblasNoTrans, 1.0, element->fir_matrix, &input_view_real.vector, 0.0, &output_sample_real.vector);
		gsl_blas_dgemv(CblasNoTrans, 1.0, element->fir_matrix, &input_view_imag.vector, 0.0, &output_sample_imag.vector);

		/*
		 * record each value in the output buffer
		 */

		output[i] = output_real[i] + I * output_imag[i];

		/*
		 * advance the input pointer
		 */

		input_view_real.vector.data++;
		input_view_imag.vector.data++;
	}
...
static unsigned tdsfilter(GSTLALComplexFIRBank *element, GstMapInfo *mapinfo, unsigned output_length)
{
	unsigned i;
	unsigned input_length;
	float *input;
...
static unsigned tdscfilter(GSTLALComplexFIRBank *element, GstMapInfo *mapinfo, unsigned output_length)
{
	unsigned i, j;
	unsigned input_length;
	float complex *input, *output;
	float *input_real, *input_imag, *output_real, *output_imag;
	gsl_vector_float_view input_view_real, input_view_imag;
	gsl_matrix_float_view output_view_real, output_view_imag;
	...
	input = g_malloc(input_length * sizeof(*input));
	output = (float complex *) mapinfo->data;
	input_real = g_malloc(input_length * sizeof(*input_real));
	input_imag = g_malloc(input_length * sizeof(*input_imag));
	output_real = g_malloc(output_length * sizeof(*output_real));
	output_imag = g_malloc(output_length * sizeof(*output_imag));
	gst_audioadapter_copy_samples(element->adapter, input, input_length, NULL, NULL);
	for(j = 0; j < input_length; j++) {
		input_real[j] = crealf(input[j]);
		input_imag[j] = cimagf(input[j]);
	}
	input_view_real = gsl_vector_float_view_array(input_real, fir_length(element));
	input_view_imag = gsl_vector_float_view_array(input_imag, fir_length(element));

	/*
	 * wrap real and imaginary parts of the output in a GSL matrix view.
	 */

	output_view_real = gsl_matrix_float_view_array(output_real, output_length, fir_channels(element));
	output_view_imag = gsl_matrix_float_view_array(output_imag, output_length, fir_channels(element));
	...
	for(i = 0; i < output_length; i++) {
		...
		gsl_vector_float_view output_sample_real = gsl_matrix_float_row(&output_view_real.matrix, i);
		gsl_vector_float_view output_sample_imag = gsl_matrix_float_row(&output_view_imag.matrix, i);
		...
		gsl_blas_sgemv(CblasNoTrans, 1.0, element->workspace.tds.working_fir_matrix, &input_view_real.vector, 0.0, &output_sample_real.vector);
		gsl_blas_sgemv(CblasNoTrans, 1.0, element->workspace.tds.working_fir_matrix, &input_view_imag.vector, 0.0, &output_sample_imag.vector);
...
static unsigned fddfilter(GSTLALComplexFIRBank *element, GstMapInfo *mapinfo, unsigned output_length)
{
	unsigned stride = fft_block_stride(element);
...
static GstFlowReturn filter_and_push(GSTLALComplexFIRBank *element, guint64 output_length)
{
...
static GstFlowReturn flush_history(GSTLALComplexFIRBank *element)
{
	GstPad *srcpad = GST_BASE_TRANSFORM_SRC_PAD(GST_BASE_TRANSFORM(element));
...
static GstFlowReturn do_new_segment(GSTLALComplexFIRBank *element)
{
	GstSegment *segment;
	gint64 samples_lost;
	GstFlowReturn result = GST_FLOW_OK;
...
enum gstlal_complexfirbank_signal {
	SIGNAL_RATE_CHANGED,
	NUM_SIGNALS
};
...
static void rate_changed(GSTLALComplexFIRBank *element, gint rate, void *data)
{
...
{
	GSTLALComplexFIRBank *element = GSTLAL_COMPLEXFIRBANK(trans);
	guint n;
...
static gboolean set_caps(GstBaseTransform *trans, GstCaps *incaps, GstCaps *outcaps)
{
	...
	gboolean success = gstlal_audio_info_from_caps(&element->audio_info, outcaps);
	const gchar *format_name = gst_structure_get_string(gst_caps_get_structure(incaps, 0), "format");
	success &= (format_name != NULL);
	...
	if(success) {
		if(!strcmp(format_name, GST_AUDIO_NE(F32))) {
			element->data_type = GSTLAL_COMPLEXFIRBANK_F32;
		} else if(!strcmp(format_name, GST_AUDIO_NE(F64))) {
			element->data_type = GSTLAL_COMPLEXFIRBANK_F64;
		} else if(!strcmp(format_name, GST_AUDIO_NE(Z64))) {
			element->data_type = GSTLAL_COMPLEXFIRBANK_Z64;
		} else if(!strcmp(format_name, GST_AUDIO_NE(Z128))) {
			element->data_type = GSTLAL_COMPLEXFIRBANK_Z128;
		} else
			g_assert_not_reached();
...
static gboolean sink_event(GstBaseTransform *trans, GstEvent *event)
{
	...
	return GST_BASE_TRANSFORM_CLASS (gstlal_complexfirbank_parent_class)->sink_event (trans, event);
}
...
{
	GSTLALComplexFIRBank *element = GSTLAL_COMPLEXFIRBANK(object);
...
static void dispose(GObject *object)
{
	...
	G_OBJECT_CLASS(gstlal_complexfirbank_parent_class)->dispose(object);
}
...
static void finalize(GObject *object)
{
	...
	G_OBJECT_CLASS(gstlal_complexfirbank_parent_class)->finalize(object);
}
...
static void gstlal_complexfirbank_class_init(GSTLALComplexFIRBankClass *klass)
{
...
static void gstlal_complexfirbank_init(GSTLALComplexFIRBank *filter)
{
