...
plot_legend = '%s:%s / %s:%s' % (ifo, options.numerator_channel_name, ifo, options.denominator_channel_name)
plt.plot(frequency, magnitude, 'r', linewidth = 0.5, label = plot_legend.replace('_', '\_'))
...
ax = plt.subplot(212)
ax.set_xscale(freq_scale)
plt.plot(frequency, phase, 'r', linewidth = 0.5)
