...
def lal_insertgap_test_03(pipeline, name):
	...
	head = pipeparts.mkgeneric(pipeline, head, "lal_insertgap", bad_data_intervals = calibration_parts.GArray(bad_data_intervals), insert_gap = False, fill_discont = True, replace_value = 0.0)
	head = pipeparts.mktee(pipeline, head)
	pipeparts.mknxydumpsink(pipeline, head, "%s_out.dump" % name)
