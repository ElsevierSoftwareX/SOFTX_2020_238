...
def channel_dict_from_channel_list(channel_list):
	...
	"""
	return dict(None if instrument_channel=="None" else instrument_channel.split("=") for instrument_channel in channel_list)
...
class GWDataSourceInfo(object):
	...
	def __init__(self, options):
		...
		self.state_channel_dict = { "H1": "GDS-CALIB_STATE_VECTOR", "H2": "GDS-CALIB_STATE_VECTOR","L1": "GDS-CALIB_STATE_VECTOR", "V1": "DQ_ANALYSIS_STATE_VECTOR" }
		self.dq_channel_dict = { "H1": "DMT-DQ_VECTOR", "H2": "DMT-DQ_VECTOR","L1": "DMT-DQ_VECTOR", "V1": None }#"DMT-DQ_VECTOR" }
		...
		if options.state_channel_name is not None:
			...
			# FIXME Is this the proper thing to do?
			if "-" in self.state_channel_dict[instrument] and "ODC_" in self.state_channel_dict[instrument].split("-")[1]:
