...
#define GST_CAT_DEFAULT gstlal_iirbank_debug
...
G_DEFINE_TYPE_WITH_CODE(
	...
	GST_DEBUG_CATEGORY_INIT(GST_CAT_DEFAULT, "lal_iirbank", 0, "lal_iirbank element")
);
...
static int push_zeros(GSTLALIIRBank *element, unsigned samples)
{
	...
	GstBuffer *zerobuf;
	// zerobuf = gst_buffer_new_allocate(NULL, samples * (element->width / 8), NULL);
...
static void set_metadata(GSTLALIIRBank *element, GstBuffer *buf, guint64 outsamples, gboolean isgap)
{
	...
	gst_buffer_set_size(buf, outsamples * iir_channels(element) * element->width / 8);
...
{
	/* GstMapInfo: New feature in Gstreamer-1.0 */
	GstMapInfo mapinfo;
	...
	input = g_malloc(available_length * sizeof(double));
	...
	output = (complex double *)&mapinfo.data;

	//output = (complex double *) GST_BUFFER_DATA(outbuf);
	g_assert(output_length * iir_channels(element) / 2 * sizeof(complex double) <= mapinfo.size);
...
static GstFlowReturn filter_s(GSTLALIIRBank *element, GstBuffer *outbuf, gboolean isgap)
{
	...
	input = g_malloc(available_length * sizeof(float));
	gst_audioadapter_copy_samples(element->adapter, input, available_length, NULL, NULL);
	...
	gst_buffer_map(outbuf, &mapinfo, GST_MAP_WRITE);
	output = (complex float *)&mapinfo.data;

	//output = (complex float *) GST_BUFFER_DATA(outbuf);
	g_assert(output_length * iir_channels(element) / 2 * sizeof(complex float) <= mapinfo.size);
