...
GstFlowReturn filter_d(GSTLALIIRBankCuda *element, GstBuffer *outbuf)
{
	...
	input = (double *)g_malloc(available_length * sizeof(double));
	...
	*/
	...
	g_assert(output_length * iir_channels(element) / 2 * sizeof(complex double) <= gst_buffer_get_size(outbuf));
	...
	gst_audioadapter_flush_samples(element->adapter, output_length * sizeof(double));
...
GstFlowReturn filter_s(GSTLALIIRBankCuda *element, GstBuffer *outbuf)
{
	...
	input = (float *)g_malloc(available_length * sizeof(float));
	gst_audioadapter_copy_samples(element->adapter, input, available_length, NULL, NULL);
...
//	gpuErrchk(cudaPeekAtLastError());
	...
	g_assert(output_length * iir_channels(element) / 2 * sizeof(complex float) <= gst_buffer_get_size(outbuf));
	...
	gst_audioadapter_flush_samples(element->adapter, output_length * sizeof(float));
	if(element->zeros_in_adapter > available_length - output_length)
	/*
	 * some trailing zeros have been flushed from the adapter
	 */
