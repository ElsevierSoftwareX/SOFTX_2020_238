...
//            atomicAdd(cudaSnr + (2 * blockIdx.x + 1) * mem_len + filt_len - 1 + i, fltrOutptImag);
		snr_index = 2 * blockIdx.x + (filt_len - 1 + i) * 2 * numTemplates;
            atomicAdd(cudaSnr + snr_index, fltrOutptReal);
            atomicAdd(cudaSnr + snr_index + 1, fltrOutptImag);
...
//                cudaSnr[(2 * cuIdG + 1) * mem_len + filt_len - 1 + i] = fltrOutptImag;
		snr_index = 2 * cuIdG + 0 + (filt_len - 1 + i) * 2 * numTemplates;
                cudaSnr[snr_index] = fltrOutptReal;
                cudaSnr[snr_index + 1] = fltrOutptImag;
            }
        }
					...
					gint queue_len)
{
	...
	unsigned int threads = blockDim.x, numFilters = blockDim.x, channels = 2 * gridDim.y;
//	printf("channels %d\n", channels);
	float data;
	COMPLEX_F *gPrevSnr;
	COMPLEX_F previousSnr;
...
//			snr_imag = &(cudaSnr[(2*by+1)*mem_len+filt_len-1+tx]);
			snr_real = &(cudaSnr[(filt_len-1+tx)*channels + 2 * by]);
			snr_imag = snr_real + 1;
		}
...
//				printf ("channelid %d, data %d %f\n", by, i+j, data);
		...
		//		__syncthreads();
			...
			if(tx < nb)
			{
				...
				snr_real[i*channels] = snrVal.re;
				snr_imag[i*channels] = snrVal.im;
			}	 
			__syncthreads();
		}
...
//	int  pos, pos_in_start = mem_in_len * by + last_sample, pos_out_start = mem_out_len * by + filt_len - 1;
	int  pos, pos_in_start = channels * last_sample + by, pos_out_start = channels * (filt_len - 1) + by;
	float *in, *out;
	int i;
	...
	// copy last to first (filt_len-1) mem data
	for (i=tx; i<filt_len - 1; i+=tdx) {
//		in = &(mem_in[mem_len * by + i]);
//		in[0] = in[len];

		in = &(mem_in[i*channels + by]);
		in[0] = in[len*channels];
	}
}
...
//  gpuErrchk (stream);
  ...
  CUDA_CHECK(cudaMemcpyAsync(out, SPSTATEUP(0)->d_mem + (SPSTATEUP(0)->filt_len -1)*SPSTATEUP(0)->channels,  SPSTATEUP(0)->channels * (spiir_processed) * sizeof(float), cudaMemcpyDeviceToHost, stream));
//  gpuErrchk (stream);
#endif
