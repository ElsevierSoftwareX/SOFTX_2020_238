...
#define GST_PACKAGE_NAME "Cool McCool"
#define GST_PACKAGE_ORIGIN "Cool McCool's Ass Crack"

/* Define PUT and GET functions for unaligned memory */
#define _GSTLAL_GET(__data, __idx, __size, __shift) \
    (((guint##__size) (((guint8 *) (__data))[__idx])) << __shift)

#define _GSTLAL_PUT(__data, __idx, __size, __shift, __num) \
 ...
 * Since: 0.10.22
 */
#define GST_READ_UINT24_BE(data)	(_GSTLAL_GET (data, 0, 32, 16) | \
					 _GSTLAL_GET (data, 1, 32,  8) | \
					 _GSTLAL_GET (data, 2, 32,  0))
 ...
 * Since: 0.10.22
 */
#define GST_READ_UINT24_LE(data)	(_GSTLAL_GET (data, 2, 32, 16) | \
					 _GSTLAL_GET (data, 1, 32,  8) | \
					 _GSTLAL_GET (data, 0, 32,  0))
...
#define GST_WRITE_UINT24_BE(data, num)	do { \
					  _GSTLAL_PUT (data, 0, 32,  16, num); \
					  _GSTLAL_PUT (data, 1, 32,  8, num); \
					  _GSTLAL_PUT (data, 2, 32,  0, num); \
					} while (0)
    ...
    GstBuffer * outbuf)
{
  ...
  in_latency = resample->funcs->get_input_latency (resample->state) + (resample->inrate >= resample->outrate); /* +1 for downsampling  0 otherwise FIXME: THIS NEEDS TO BE MORE CARFUL for non power of 2*/
   ...
   */
  skipGAP = in_len > in_latency && GST_BUFFER_FLAG_IS_SET(inbuf, GST_BUFFER_FLAG_GAP);
  ...
  if ( skipGAP ) {
    ...
    if (need_convert)
      err = resample->funcs->process (resample->state, in_tmp, &in_processed, out_tmp, &out_processed);
    else
      err = resample->funcs->process (resample->state, (const guint8 *) GST_BUFFER_DATA (inbuf), &in_processed, (guint8 *) GST_BUFFER_DATA (outbuf), &out_processed);
  }
...
plugin_init (GstPlugin * plugin)
{
  ...
  if (!gst_element_register (plugin, "gstlal-audioresample", GST_RANK_PRIMARY,
