...
static void *read_series(GSTLALFrameSrc *element, guint64 offset, guint64 length)
{
	...
	GST_LOG_OBJECT(element, "reading %llu samples (%g seconds) of channel \"%s\" at %d.%09u s", length, (double) length / element->rate, element->full_channel_name, start_time.gpsSeconds, start_time.gpsNanoSeconds);
	...
	case LAL_I4_TYPE_CODE:
		...
		if(!series) {
			GST_ERROR_OBJECT(element, "XLALFrReadINT4TimeSeries() %llu samples (%g seconds) of channel \"%s\" at %d.%09u s failed: %s", length, (double) length / element->rate, element->full_channel_name, start_time.gpsSeconds, start_time.gpsNanoSeconds, XLALErrorString(XLALGetBaseErrno()));
			XLALClearErrno();
			return NULL;
		}
		break;
	...
	case LAL_S_TYPE_CODE:
		...
		if(!series) {
			GST_ERROR_OBJECT(element, "XLALFrReadREAL4TimeSeries() %llu samples (%g seconds) of channel \"%s\" at %d.%09u s failed: %s", length, (double) length / element->rate, element->full_channel_name, start_time.gpsSeconds, start_time.gpsNanoSeconds, XLALErrorString(XLALGetBaseErrno()));
			XLALClearErrno();
			return NULL;
		}
		break;
	...
	case LAL_D_TYPE_CODE:
		...
		if(!series) {
			GST_ERROR_OBJECT(element, "XLALFrReadREAL8TimeSeries() %llu samples (%g seconds) of channel \"%s\" at %d.%09u s failed: %s", length, (double) length / element->rate, element->full_channel_name, start_time.gpsSeconds, start_time.gpsNanoSeconds, XLALErrorString(XLALGetBaseErrno()));
			XLALClearErrno();
			return NULL;
		}
		break;
